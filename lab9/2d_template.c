...
int read_char_lines(char *array[]) {
    ...
    while(fgets(line_buffer, BUF_SIZE, stdin)) {
        ...
        array[line_count] = malloc(line_length + 1);
        strcpy(array[line_count], line_buffer);
        line_count++;
    }
...
void write_char_line(char *array[], int index) {
    if (array[index]) {
        printf("%s\n", array[index]);
    }
}
        ...
        if (ch == ' ' || ch == '\n') {
            if (index > 0) {
                ...
                *size = *size + 1;
                ...
                line = realloc(line, (*size) * sizeof(int));
                line[*size - 1] = num;
                index = 0;
            }
        } else {
